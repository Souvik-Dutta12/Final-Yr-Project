...
export default function MapSection({
  ...
  useEffect(() => {
    ...
    // Clipping pane making
    map.createPane("clippedPane");
    map.getPane("clippedPane").style.zIndex = 450;
    ...
    map.on("click", async (e) => {
      ...
      marker
        .bindPopup(
          `<div style="min-width:200px"><b>📍 Loading…</b><br/><small style="color:#6b7280;font-family:monospace">${lat.toFixed(5)}, ${lng.toFixed(5)}</small></div>`,
        )
        .openPopup();
    ...
    map.on(L.Draw.Event.CREATED, async (event) => {
      ...
      // ── Draw farmland overlay ──
      const farmGeoJson =
        farmRes.status === "fulfilled" ? farmRes.value?.data : null;
      if (farmGeoJson?.features) {
        analysisLayerRef.current.clearLayers();

        // Make GeoJSON of Drawn polygon 
        const drawnGeoJson = {
          type: "Feature",
          geometry: {
            type: "Polygon",
            coordinates: [coords.map((c) => [c[1], c[0]])], // [lng, lat]
          },
          properties: {},
        };

        // Show Drawn polygon as farmland base 
        L.geoJSON(drawnGeoJson, {
          style: {
          ...
          .bindTooltip("Farmland", { sticky: true })
          .addTo(analysisLayerRef.current);

        // Water and builtup —   clip coords of backend in drawn polygon
        const drawnPolygonLayer = L.geoJSON(drawnGeoJson);
        const drawnBounds = drawnPolygonLayer.getBounds();

        farmGeoJson.features.forEach((feat) => {
          const label = feat.properties?.class || "unknown";
          if (label === "farmland") return;

          const color = LAND_COLORS[label] || "#888";
          const geoJsonCoords = feat.geometry?.coordinates;
          if (!geoJsonCoords) return;

          // coords swap [lng,lat] → [lat,lng]
          const swapped = geoJsonCoords.map((ring) =>
            ring.map((pt) => [pt[1], pt[0]]),
          );

          // check if bounds of feature is in drawn polygon
          const featLayer = L.polygon(swapped);
          const featBounds = featLayer.getBounds();

          const center = featBounds.getCenter();
          if (!drawnBounds.contains(center)) return;
        ...
        try {
          ...
          L.geoJSON(soilGeoJson, {
            ...
            onEachFeature: (feature, layer) => {
              const soilClass = feature.properties?.soil_class;
              if (soilClass) layer.bindTooltip(soilClass, { sticky: true });
            },
      ...
      const landRows = Object.entries(landCount)
        .map(
          ([label, count]) => `
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
